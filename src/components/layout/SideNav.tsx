...
function SideNav() {
  ...
  return (
    <>
      ...
      <button onClick={toggleMenu} className="md:hidden fixed top-4 left-4 z-50 p-2 bg-[#4461F2] text-white rounded-md">
        {isOpen ? "Close" : "Menu"}
      </button>
        ...
        <div className="p-4 w-max">
          ...
          <ul className="mt-8 space-y-2">
            ...
            <li>
              <Link
                onClick={toggleMenu}
                className={`rounded-[10px] flex items-center gap-10 p-2 ${segment == null ? "bg-[#4461F2] text-white" : ""}`}
                href="/">
                <span>
            ...
            {/* Quiz History */}
            <li>
              <Link onClick={toggleMenu} className="rounded-[10px] flex items-center gap-10 p-2" href="/">
                <span>
      ...
      {/* Backdrop on mobile */}
      {isOpen && <div onClick={toggleMenu} className="fixed inset-0 bg-black/30 md:hidden z-30" />}
    </>
  );
}
